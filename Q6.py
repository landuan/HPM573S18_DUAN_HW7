...
class MultiCohort:
    ...
    def simulate(self, n_time_steps):
        ...
        for i in range(len(self._ids)):
            ...
            #store five
            self.five.append(sum(cohort._five))
...
class Calibration:
    ...
    def sample_posterior(self):
        ...
        self.weights=Stat.binom.pmf(k=CLIN_RESULTS,n=POP_SIZE,p=multiCohort.percentages)
...
#PROJECTION INTERVAL
X=Calimodel._meanSurvivalTimes
sum_statX = StatSupport.SummaryStat('X samples', X)
estimatex=sum_statX.get_mean()
projection_interval=sum_statX.get_PI(0.05)
print('Estimate of mean survival times ({:.{prec}%} projection interval):'.format(1-0.05, prec=0),FormatSupport.format_estimate_interval(estimatex,projection_interval,4))
print("We can see that the estimate of mortality probability and mean survival times do not change too much but the 95% credible interval and projection inverval become shorter because of larger sample.")
